function createPost(
            ...
            <div class="cardfooter" id="cardfooter">
              ...
              <div class="tags ${
                tags.length !== 0 ? "domargin" : ""
                  ...
                  ${tags
                    .map((tag) => `<p class="tag">${tag.name}</p>`)
                    .join("")}
              </div>
            </div>
        </div>  
  ...
  if (userPosts) {
    document.querySelector("#profile-posts").append(card);
  } else {
...
function showpost(id) {
  // window.removeEventListener("scroll", pagination); // to do
  axios
    .get(`https://tarmeezacademy.com/api/v1/posts/${id}`)
    .then((response) => {
      document.getElementById("profile-posts").innerHTML = ""; // to do
...
//     currentPage++;
//     getPosts(currentPage, true, false);
...
// window.addEventListener("scroll", pagination); // to do
  ...
  axios
    ...
    .then((response) => {
      ...
      if (document.getElementById("profile-posts")) {
        if (document.getElementById("profile-posts").children.length === 1) {
          if (document.querySelector(".create-comment-container")) {
            document.querySelector(".create-comment-container").style.display =
              "flex";
          }
